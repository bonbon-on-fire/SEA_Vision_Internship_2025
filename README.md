...
- **Location:**  
  - Likely in `src/cpp/operations/cpp/OperationFactory.cpp` and `hpp/OperationFactory.hpp`
...
- **Location:**  
  - Generated by CLI, saved in `pipeline_cli.json` or in `tests/json/`
...
- **Location:**  
  - `src/cpp/graph/cpp/` and `src/cpp/graph/hpp/`
...
- **C++ Tests:**  
  - `tests/cpp/test_operations.cpp` — Unit tests for each operation.
  - `tests/cpp/test_runner.cpp` — End-to-end pipeline tests.

- **Python Tests:**  
  - `tests/python/test_sequence.py` — CLI and config tests.

- **Test Configs:**  
  - `tests/json/` — Example pipeline configs for automated testing.
